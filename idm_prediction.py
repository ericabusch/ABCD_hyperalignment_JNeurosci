...
from utils import n_jobs, n_parcels


def prediction_pipeline_region(align, scale, parcel, y_name, y, overwrite=False, n_folds=None):
    outfn = f'{outdir}/{y_name}_{align}_{scale}_parcel_{parcel:03d}_COV.npz'
    ...
    np.savez(outfn, yhat=yhat, clf_info=clf_info, r2=r2)
...
def prediction_pipeline(y_name, align, scale, subjects, overwrite=False, n_jobs=1, n_folds=None):
    ...
    """
    y, _ = get_measure_info(y_name, subjects)
    jobs = []
    for parcel in range(1, n_parcels+1):
        jobs.append(delayed(prediction_pipeline_region)(
            align, scale, parcel, y_name, y, overwrite=overwrite, n_folds=n_folds
        ))
...
if __name__ == '__main__':
    ...
    outdir = f'{utils.abcd_dir}/results/prediction/COV_pred/'
